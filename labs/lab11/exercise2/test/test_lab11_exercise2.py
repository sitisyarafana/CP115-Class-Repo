...
def test_no_danger_days():
    ...
    assert danger_days == expected_danger_days, f"Input: {inputs} | Expected danger_days: {expected_danger_days} | Got: {danger_days}"
    assert abs(average_temp - expected_average) < 0.1, f"Input: {inputs} | Expected average_temp: {expected_average} | Got: {average_temp}"
