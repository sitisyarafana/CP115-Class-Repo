...
def test_basic_scores_no_bonus():
    ...
    assert final_score == expected_total, f"Input: {inputs} | Expected final_score: {expected_total} | Got: {final_score}"
    assert rounds_processed == 3, f"Input: {inputs} | Expected rounds_processed: 3 | Got: {rounds_processed}"
...
def test_single_score_with_bonus():
    ...
    assert final_score == expected_total, f"Input: {inputs} | Expected final_score: {expected_total} | Got: {final_score}"
    assert rounds_processed == 1, f"Input: {inputs} | Expected rounds_processed: 1 | Got: {rounds_processed}"
...
def test_mixed_scores_with_bonus():
    ...
    assert final_score == expected_total, f"Input: {inputs} | Expected final_score: {expected_total} | Got: {final_score}"
    assert rounds_processed == 4, f"Input: {inputs} | Expected rounds_processed: 4 | Got: {rounds_processed}"
...
def test_zero_rounds():
    ...
    expected_total = 0.0
    assert final_score == expected_total, f"Input: {inputs} | Expected final_score: {expected_total} | Got: {final_score}"
    assert rounds_processed == 0, f"Input: {inputs} | Expected rounds_processed: 0 | Got: {rounds_processed}"
...
def test_many_rounds():
    ...
    assert final_score == expected_total, f"Input: {inputs} | Expected final_score: {expected_total} | Got: {final_score}"
    assert rounds_processed == 5, f"Input: {inputs} | Expected rounds_processed: 5 | Got: {rounds_processed}"
...
def test_alternating_bonus():
    ...
    assert final_score == expected_total, f"Input: {inputs} | Expected final_score: {expected_total} | Got: {final_score}"
    assert rounds_processed == 6, f"Input: {inputs} | Expected rounds_processed: 6 | Got: {rounds_processed}"
...
def test_decimal_bonus_calculation():
    ...
    assert final_score == expected_total, f"Input: {inputs} | Expected final_score: {expected_total} | Got: {final_score}"
    assert rounds_processed == 2, f"Input: {inputs} | Expected rounds_processed: 2 | Got: {rounds_processed}"